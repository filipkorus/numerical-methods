...
if(0) % Metoda #1
    ...
    while( x < realmax ) % jesli x mniejsze od wartosci najwiekszej, to ...
        n=n+1,
        x=2*x; % zwieksz licznik, pomnoz x przez dwa = zwieksz wykladnik o 1
...
if(1) % Metoda #2: x=1, mnozenie razy 2
    ...
    num2bitstr( single(x) ); pause % reprezentacja bitowa
    while(single(2*x) > x) % czy widac mnozenie przez 2; jesli tak, to
        n=n+1, x=single(2*x), % zwieksz wykladnik o 1, pomnoz *2,
