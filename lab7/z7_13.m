...
%%%%podczas calkowania funkcji gaussowskiej powinna nam wyjsc wartosc
%%%%bliska 0- i taka wychodzi w wiekszosci przypadkow
%błedy- w wiekszosci przypadkow sie zgadzaja, poprawne wywolania funkcji-
%nie zgadzają sie glownie liczby bardzo dlugo po przecinku juz
clear all; close all;

c_erf = erf( 3/sqrt(2) ), % wartosc obliczona przez Matlaba funkcja erf zwraca 1. Przyblizenie szukanej wartosci,
f = inline( '1/sqrt(2*pi)*exp(-x.^2/2)' ); % calkowana funkcja
h =0.6; % odleglosc pomiedzy wezlami ----wzraz ze wrostem h rosnie błąd- przy h=6 bład rzedu juz nawet 0,9 w niektorych funkcjach
...
[ abs(c_trapez-c_erf); abs(c_simpson13-c_erf); abs(c_gauss3-c_erf);
    abs(c_gauss33-c_erf); abs(c_quad-c_erf); abs(c_quadl-c_erf) ]
